...
public class HighLowGUI extends Application {
    ...
    private BlackjaceHand dealerHand;      // The cards that have been dealt so far.
    ...
    public void start(Stage stage) {
        ...
        higher.setOnAction( e -> doHit() );
        Button lower = new Button("Stay");
        lower.setOnAction( e -> doStay() );
...
//        HBox.setHgrow(newGame, Priority.ALWAYS); // not the same size.
    ...
    private void doHit() {
        ...
        playerHand.addCard( deck.dealCard() );     // Deal a card to the hand.
...
//        }
        ...
        Card prevCard = dealerHand.getCard( cardCt - 2 );  // The previous card.
        if ( thisCard.getValue() > prevCard.getValue() ) {
            gameInProgress = false;
            message = "Too bad! You lose.";
        }
    ...
    private void doNewGame() {
        ...
        dealerHand = new BlackjaceHand();
        deck.shuffle();
        dealerHand.addCard( deck.dealCard() );  // Deal the first card into the hand.
        message = "Hit or Stay? : " + playerHand.getBlackjackValue();
        gameInProgress = true;
        drawBoard();
    } // end doNewGame()
    ...
    private void drawBoard() {
        ...
        for (int i = 0; i < cardCt; i++)
            drawCard(g, dealerHand.getCard(i), 20 + i * 99, 20);

        // player
        for (int i = 0; i < cardCt; i++)
            drawCard(g, dealerHand.getCard(i), 20 + i * 99, 183);
